...
// ======================================================================
// ========= HERRAMIENTA TEMPORAL PARA MIGRAR DATOS ANTIGUOS ==========
// ======================================================================
app.get('/api/migrate-formalized-data', requireLogin, checkRole(['Administrador']), async (req, res) => {
    const client = await pool.connect();
    try {
        await client.query('BEGIN');

        // 1. Buscamos todas las cotizaciones que ya estaban marcadas como 'formalizada'
        const oldFormalizedQuotes = await client.query(
            "SELECT id, quotenumber, clientname, advisorname, createdat FROM quotes WHERE status = 'formalizada'"
        );

        if (oldFormalizedQuotes.rows.length === 0) {
            return res.send("No se encontraron cotizaciones antiguas para migrar.");
        }

        let migratedCount = 0;

        // 2. Por cada cotización antigua, creamos un registro en la nueva tabla
        for (const quote of oldFormalizedQuotes.rows) {
            // Buscamos el ID del centro para tener la referencia correcta
            const centerResult = await client.query('SELECT id FROM centers WHERE name = $1', [quote.clientname]);
            
            if (centerResult.rows.length > 0) {
                const centerId = centerResult.rows[0].id;

                // Insertamos el registro en la nueva tabla, evitando duplicados si la migración se corre más de una vez.
                // ON CONFLICT(center_id) DO NOTHING asegura que no se creen entradas duplicadas para el mismo centro.
        ...
        await client.query('COMMIT');
        res.status(200).json({ message: `¡Migración completada! Se añadieron ${migratedCount} registros históricos a la tabla de formalizados.` });

    } catch (err) {
        await client.query('ROLLBACK');
        console.error('Error durante la migración de datos:', err);
        res.status(500).json({ message: 'Error en el servidor durante la migración.' });
    } finally {
        client.release();
    }
});
